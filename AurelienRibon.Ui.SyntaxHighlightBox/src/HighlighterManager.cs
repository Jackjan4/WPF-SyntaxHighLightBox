...
namespace AurelienRibon.Ui.SyntaxHighlightBox
{
    public class HighlighterManager
    {
        ...
        private HighlighterManager() {
            Highlighters = GetInternalResources();
        }

        private IDictionary<string, IHighlighter> GetInternalResources() {
            IDictionary<string, IHighlighter> result = new Dictionary<string, IHighlighter>();
            ...
            foreach (var res in GetResources("resources/(.+?)[.]xml")) {
                ...
                result.Add(name, new XmlHighlighter(root));
            }
        ...
        public void LoadSyntaxXML(String xmlContent) {
            using (Stream s = GenerateStreamFromString(xmlContent)) {
                LoadSyntaxXML(s);
            }
        ...
        public void LoadSyntaxXML(Stream xmlContent) {
            ...
            try {
                ...
                XmlReader reader = XmlReader.Create(xmlContent, readerSettings);
                xmldoc = XDocument.Load(reader);

            } catch (XmlSchemaValidationException ex) {
                Debug.WriteLine("Xml validation error at line " + ex.LineNumber + " for " + " :");
                Debug.WriteLine("Warning : if you cannot find the issue in the xml file, verify the xsd file.");
                Debug.WriteLine(ex.Message);
                return;
            } catch (Exception ex) {
